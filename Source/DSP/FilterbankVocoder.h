...
private:
    ...
    //  【修正H 2026-08-02】キャリアの帯域別正規化を入れたことで全体レベルが上がるため
    //  8.0f → 3.54f へ再校正 (既定 48band/RESO1.0 で有音部の中央値が修正前と一致)。
    static constexpr float kBpfMakeup = 3.54f;
    ...
    // ---- 【修正H 2026-08-02】キャリアの帯域別正規化 --------------------
    //  旧実装は out = キャリア帯域出力 × モジュレーター包絡 と「掛ける」だけで、
    //  キャリア自身の帯域レベルで割っていなかった。そのため鋸波キャリアの
    //  -6dB/oct がそのまま出力に残り、実測で母音の 5-8kHz が -35.1dB、
    //  無声音では低域 +10.8dB / 5-8kHz -38.0dB とバランスが崩れていた。
    //  本来のチャンネルボコーダーはキャリア帯域を正規化してから包絡を掛けるので、
    //  キャリアが鋸波でも和音でも Wavetable でもモジュレーターの音色が正しく出る。
    //
    //  ゼロ割防止と「キャリアが鳴っていない帯域の暗騒音を持ち上げすぎない」ため、
    //  帯域包絡には広帯域キャリアレベルに対する相対フロアを課す。
    //  フロアは実測で決定 (母音の 5-8kHz 誤差: 0.02→-10.3dB / 0.008→-3.9dB / 0.003→-3.3dB)。
    //  下げるほど高域は正確になるがキャリアの居ない帯域の暗騒音も持ち上がるため 0.008 とする。
    static constexpr float kCarFloorRel = 0.008f;   // 広帯域レベルの -42dB
    static constexpr float kCarMaxBoost = 1000.0f;  // 数値保護の上限 (通常はフロア側が効く)
    std::array<float, kMaxBands> mCarEnv {};       // 帯域毎キャリア包絡 (L/R平均)
    float mCarBroad = 0.0f;                        // 広帯域キャリア包絡
    float mCarAttCoef = 0.0f;                      // 追従係数 att 1ms (ゲインを絞る側は速く)
    float mCarRelCoef = 0.0f;                      // 追従係数 rel 100ms
};
