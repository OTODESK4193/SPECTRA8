...
public:
    ...
    // 項目ごとの有効／無効判定 (引数は itemId)。false を返すと非活性化 (選択不可) になる。
    void setItemEnabledPredicate(std::function<bool(int)> predicate) { mItemEnabledPredicate = std::move(predicate); }
    ...
    void showPopup() override
    {
        ...
        for (int i = 0; i < getNumItems(); ++i)
        {
            ...
            item.isTicked = tick;
            item.customComponent = new HelpItem(*this, getItemText(i), help, tick, enabled);
...
private:
    ...
    public:
        ...
        HelpItem(juce::Component& ownerBox, juce::String itemText,
                 juce::String helpText, bool ticked, bool enabled)
            : juce::PopupMenu::CustomComponent(enabled),   // enabled == false なら自動確定しない
              mOwner(&ownerBox),
        ...
        {
            if (!mEnabled) shouldBeHighlighted = false;
        ...
        void paint(juce::Graphics& g) override
        {
            ...
            if (mTicked)
            {
                g.setColour(mEnabled ? SpectraColors::mint : SpectraColors::textDim.withAlpha(0.3f));
            ...
            if (!mEnabled)
            {
                g.setColour(SpectraColors::textDim.withAlpha(0.28f));
